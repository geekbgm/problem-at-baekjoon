...
queue<pair<int, int>> Swan_Q, Swan_NQ, Q, NQ; // Swan_Q는 백조에 대한 queue, Q는 물에 대한 queue를 의미한다.
pair<int, int> Swan_Pos;// 가장 끝에있는 백조의 위치를 나타낸다. (백조 하나만 움직이는 것임 나머지는 고정)
...
void Swan_BFS()
{
    while (Swan_Q.empty() == 0 && Find == false)
    {
        ...
        for (int i = 0; i < 4; i++) //가능한 포지션을 모두 움직이는 것
        {
            int nx = x + dx[i];
            int ny = y + dy[i];
